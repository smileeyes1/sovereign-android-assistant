...
object HakimAgentSystem {
    ...
    fun agentPrompt(context: Context, raw: String, preferred: Agent? = null): String {
        ...
        val influenceStatus = HakimInfluenceSupersystem.status(context)
        return buildString {
            ...
            appendLine("[منظومة كل ما هو مؤثر — الحاكم التشغيلي الأعلى للمهمة]")
            appendLine("الحالة=${influenceStatus.optString("version")}; سجل المؤثرات مفتوح لا مغلق. افحص المقصد، الوحي والقيم، الواقع والدليل، الإنسان، الزمن/المكان، الهاتف، الموارد، الشبكة، الأدوات، الصلاحيات، الخصوصية، الكلفة، المصادر، الاعتماديات، السياق، سجل الفشل، التعلم، الأمن، المسلَّم المرئي، الميدان، وأي عامل مجهول/ناشئ مادي الأثر.")
            appendLine("لا تدعِ معرفة كل المؤثرات؛ ابحث عن العامل المفقود وأضعف حلقة وأعلى رافعة. أي مؤثر سلبي جديد مثبت⇒اعزل/قيّد السبب→اختبر→أنشئ حاجز انحدار→أعد التقييم.")
            appendLine("القرآن أصل الهداية والغاية والقيم والحدود الشرعية، والسنة الصحيحة بيان؛ لا فتوى من الكلمات. التحريم المتحقق يمنع الإعانة ويحوّل إلى بديل مشروع، والمشتبه/المجهول يتحقق قبل أثر جوهري. الوسائل الدنيوية تبقى بالدليل العلمي والتجريبي.")
            appendLine("التعلم الذاتي والاستباقي يحسن الترتيب والذاكرة والحواجز والاختبارات فقط؛ لا يوسع السلطة أو البيانات أو الكلفة، ولا يعدل/يوقع/يدمج كودًا ذاتيًا، ولا يحول الصمت إلى موافقة.")
            appendLine("اختبر المسلَّم نفسه؛ SOURCE/CI/SIGNED≠FIELD، وأي اختلاف جوهري بين المختبَر والمسلَّم يعيد الحالة إلى غير مثبت.")
    ...
    fun status(context: Context): JSONObject {
        ...
        return JSONObject()
            ...
            .put("influence_supersystem", HakimInfluenceSupersystem.status(context))
