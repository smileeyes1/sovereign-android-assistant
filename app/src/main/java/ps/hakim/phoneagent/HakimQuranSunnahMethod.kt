...
object HakimQuranSunnahMethod {
    ...
    fun assess(raw: String): Assessment {
        ...
        return Assessment(
            ...
            exactSourceVerification = quranic.exactQuranTextRequired || religious.exactSourceRequired,
            worldlyEvidenceRequired = quranic.worldlyMeansTask,
            reason = when {
                quranic.exactQuranTextRequired || religious.exactSourceRequired -> "يلزم التثبت من النص/النسبة قبل الجزم"
    ...
    fun promptContext(raw: String): String {
        ...
        return buildString {
            ...
            appendLine("القرآن كتاب مبارك؛ وعند طلب «بركة القرآن» افهم المقصود على وجهه الشرعي: الانتفاع به بالإيمان والتلاوة والتدبر والذكر والاهتداء والعمل بما دل عليه وتعليمه على الوجه المشروع، مع رد تفاصيل الفضائل والآثار المخصوصة إلى دليلها وعدم اختلاق وعد أو أثر لم يثبت.")
            appendLine("لا تحوّل القرآن أو السنة أو البركة أو الدعاء إلى خوارزمية تقنية أو ضمان نتيجة مادية. ولا تعامل البركة كقوة تقنية خفية يمكن للبرنامج قياسها أو تشغيلها أو ضمان آثارها. الطب والهندسة والفيزياء والبرمجة والإدارة وسائر الوسائل تُحسم بأقوى دليل وخبرة متاحة داخل الحدود الشرعية والأخلاقية.")
            ...
            if (a.exactSourceVerification) appendLine("هذه المهمة تحتاج تحققًا نصيًا/مصدرًا موثوقًا قبل أي نسبة جازمة إلى القرآن أو السنة.")
    ...
    fun status(): JSONObject = JSONObject()
        ...
        .put("quran_is_blessed", true)
        .put("quranic_barakah_is_lawful_benefit_not_technical_guarantee", true)
        .put("specific_virtues_and_effects_require_evidence", true)
