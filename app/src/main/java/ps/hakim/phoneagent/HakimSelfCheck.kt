...
object HakimSelfCheck {
    ...
    fun run(context: Context): JSONObject {
        ...
        check("لا إله إلا الله — التوحيد مثبت", tawhidAla.optBoolean("la_ilaha_illa_allah_tawhid"), "fail")
        check("محمد رسول الله — الرسالة مثبتة", tawhidAla.optBoolean("muhammad_is_messenger_of_allah"), "fail")
        check("لا وحي جديد من نظام أو نموذج", tawhidAla.optBoolean("no_new_revelation_from_model_or_system"), "fail")
        check("سورة الأعلى بوصلة قيمية خاصة", tawhidAla.optBoolean("surah_al_ala_special_compass"), "fail")
        check("نص الأعلى الدقيق يحتاج corpus متحقق", tawhidAla.optBoolean("surah_al_ala_exact_text_requires_verified_corpus"), "fail")
        check("سورة الأعلى ليست خوارزمية تقنية", tawhidAla.optBoolean("surah_al_ala_not_technical_algorithm"), "fail")
        check("البرنامج لا يقيس الحالة الروحية", tawhidAla.optBoolean("spiritual_state_not_measurable_by_software"), "fail")
        check("المرجعية المعيارية للقرآن مثبتة", normative.optBoolean("normative_authority_quran"), "fail")
        check("المرجعية المعيارية للسنة الصحيحة مثبتة", normative.optBoolean("normative_authority_authentic_sunnah"), "fail")
        check("لا مصدر وحي معياري ثالث", !normative.optBoolean("other_normative_revelation_sources"), "fail")
        check("التفسير أداة فهم لا وحي مستقل", normative.optBoolean("tafsir_is_human_interpretive_aid_not_revelation"), "fail")
        check("الفقه فهم بشري لا وحي مستقل", normative.optBoolean("fiqh_is_human_juristic_understanding_not_revelation"), "fail")
        check("العلم دليل للوسيلة لا مصدر وحي", normative.optBoolean("worldly_science_is_evidence_for_means_not_revelation"), "fail")
        check("الإنسان مالك المقصد والبيانات والقرار المأذون", normative.optBoolean("human_user_is_owner_of_intent_data_and_authorized_decision"), "fail")
        check("حكيم وكيل واحد داخل نطاقه", normative.optBoolean("hakim_is_single_authorized_agent_inside_its_scope"), "fail")
        check("لا ادعاء ملكية Android أو العتاد", normative.optBoolean("hakim_does_not_claim_ownership_of_android_or_hardware"), "fail")
        ...
        check("المرجعية السيادية مدمجة في نسيج التكامل", structural.optBoolean("normative_sovereignty_integrated"), "fail")
        check("بوصلة التوحيد والأعلى مدمجة", structural.optBoolean("tawhid_ala_compass_integrated"), "fail")
