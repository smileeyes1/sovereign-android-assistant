...
object HakimSelfLeadershipController {
    ...
    fun evaluate(context: Context, goal: String): State {
        ...
        return State(mode, reason, phase?.name ?: "IDLE", accessibility, browser, inAppReasoning, meshExecution, best?.node?.id ?: "none", best?.score ?: 0)
    }
    ...
    fun promptContext(context: Context, goal: String): String {
        ...
        return buildString {
            ...
            appendLine("الاستدلال الداخلي=${s.inAppReasoningReady}، المتصفح الحاضر=${s.browserReady}، الوصول الشامل=${s.accessibilityReady}. لا تجعل Accessibility شرطًا إذا كان مسار أقل صلاحية يحقق الغاية.")
            ...
            appendLine("عند فشل أداة، انتقل تلقائيًا إلى البديل التالي المسموح في شبكة التفوق بدل إعادة المحاولة بلا تقدم.")
