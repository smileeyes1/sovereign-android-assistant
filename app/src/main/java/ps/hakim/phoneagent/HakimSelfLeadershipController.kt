...
object HakimSelfLeadershipController {
    ...
    fun evaluate(context: Context, goal: String): State {
        ...
        val mode = when {
            ...
            looksLikeInteractiveTask(goal) && !accessibility -> Mode.RECOVER_CAPABILITY
        ...
        val reason = when (mode) {
            ...
            Mode.RECOVER_CAPABILITY -> "القدرة التنفيذية المطلوبة غير جاهزة؛ أصلح/استعد القدرة أو بدّل المسار دون توسيع السلطة"
        ...
        return State(mode, reason, phase?.name ?: "IDLE", accessibility, browser)
    }
