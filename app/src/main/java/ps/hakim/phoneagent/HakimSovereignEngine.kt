...
/**
 * طبقة تجميع سيادية: مقصد + مصفوفة قرار + نزاهة شرعية + سجل مهمة + ميزانية فشل.
...
object HakimSovereignEngine {
    ...
    fun promptContext(
        ...
        return buildString {
            ...
            appendLine("سلسلة الاستقلالية: افهم→ثبّت العقد→قدّر الواقع→اختر المسار→نفّذ أقل خطوة كافية→تحقق من الأثر→أصلح السبب→أعد التقدير→أغلق أو توقف عند بوابة لازمة.")
    ...
    fun status(context: Context): JSONObject = JSONObject()
        ...
        .put("independence_policy", "أقصى استقلالية آمنة ومشروعة ومصرح بها مع تحقق وأثر مثبت")
}
