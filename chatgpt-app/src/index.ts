...
app.get("/privacy",(_req,res)=>res.type("html").send(`<!doctype html><html lang="ar" dir="rtl"><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>خصوصية حكيم</title><body><h1>خصوصية حكيم</h1><h2>البيانات التي يعالجها حكيم</h2><p>يعالج الجسر الحد الأدنى اللازم للربط والتنفيذ: رموز OAuth والاقتران المشفرة، حالة اتصال مختصرة للجهاز، هدف فتح يحدده المستخدم (اسم حزمة تطبيق أو رابط HTTPS)، وأوامر التنقل العامة Home/Back/Recents. في النسخة العامة لا تُعاد إلى ChatGPT لقطات الشاشة الخام أو الإشعارات أو الكتابة الحرة أو النقر العام.</p><h2>الغرض</h2><p>تستخدم هذه البيانات حصريًا لربط جهاز مأذون بحساب ChatGPT وتنفيذ الطلب الذي اختاره المستخدم والتحقق من اكتماله. لا تستخدم للإعلانات أو بناء ملفات تعريف تسويقية.</p><h2>المستلمون والنقل</h2><p>ChatGPT يعالج المحادثة وفق إعدادات حساب المستخدم وسياسات OpenAI. أوامر الجهاز ونتائجه تنتقل عبر ناقل خارجي بصيغة مشفرة طرفًا لطرف؛ الناقل لا يحتاج إلى رؤية المحتوى الصريح. الاستضافة الحالية على Railway تشغّل خادم OAuth/MCP ولا تحتاج مفتاح OpenAI API.</p><h2>الاحتفاظ</h2><p>رموز التفويض المؤقتة تحفظ في التخزين الدائم فقط للمدة اللازمة لإكمال OAuth ثم تُستهلك وتحذف؛ صلاحية authorization code دقيقتان. access tokens صالحة ساعة، وrefresh tokens حتى ٣٠ يومًا ما لم يُفصل الربط أو تتغير الأسرار. لا يحتفظ الجسر افتراضيًا بنسخ دائمة من محتوى شاشة الجهاز أو الإشعارات.</p><h2>التحكم</h2><p>يمكن للمستخدم إلغاء الربط أو إعادة الاقتران. الأفعال التي تغيّر حالة الهاتف تبقى خلف موافقة Android. لا توجد قناة shell أو root، ولا يتم توسيع صلاحيات Android خارج ما وافق عليه المستخدم والنظام.</p><h2>الأمان وتقليل البيانات</h2><p>الأوامر والنتائج مشفرة طرفًا لطرف، والنسخة العامة تعيد أقل نتيجة لازمة للمقصد. لا تُعرض أسرار المصادقة أو مفاتيح الربط في نتائج الأدوات.</p></body></html>`));
