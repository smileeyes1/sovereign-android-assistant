...
",encoding="utf-8")
    ...
    def event(self,kind,source,value):
        ...
        with self.conn() as c:
            c.execute("INSERT INTO events(ts,kind,source,value,evidence_sha256) VALUES(?,?,?,?,?)",
    ...
    def put_command(self,intent,params,idempotency_key):
        ...
        if not intent: raise ValueError("EMPTY_INTENT")
        params=params if isinstance(params,dict) else {}
        canonical=json.dumps({"intent":intent,"params":params,"idempotency_key":str(idempotency_key)},sort_keys=True,separators=(",",":"),ensure_ascii=False)
        cid=hashlib.sha256(canonical.encode()).hexdigest()
        t=now_ms()
        with self.conn() as c:
            c.execute("""INSERT OR IGNORE INTO commands(id,created_at,updated_at,intent,params_json,status)
                         VALUES(?,?,?,?,?,?)""",(cid,t,t,intent,json.dumps(params,ensure_ascii=False,sort_keys=True),"queued"))
        self.event("MISSION","api",f"queued:{cid}")
        return cid
    def get_command(self,cid):
        with self.conn() as c:
            r=c.execute("SELECT id,created_at,updated_at,intent,params_json,status,result_json FROM commands WHERE id=?",(cid,)).fetchone()
        if not r: return None
        return {"id":r[0],"created_at":r[1],"updated_at":r[2],"intent":r[3],"params":json.loads(r[4]),"status":r[5],"result":json.loads(r[6])}
    def counts(self):
        with self.conn() as c:
...
def model_status():
    try:
        ...
        return {"ready":False,"model":"","endpoint":"127.0.0.1:8080","error":e.__class__.__name__[:80]}
...
class Api(BaseHTTPRequestHandler):
    ...
    def _auth(self):
        ...
        return hmac.compare_digest(got,self.server.store.token)
    def _body(self):
    ...
    def do_GET(self):
        if self.client_address[0] not in ("127.0.0.1","::1"): return self._json(403,{"ok":False,"error":"LOOPBACK_ONLY"})
        if self.path=="/health":
            e,q=self.server.store.counts()
            return self._json(200,{"ok":True,"version":VERSION,"owner":"human_user","loopback_only":True,"external_provider_required":False,"event_count":e,"queued_commands":q})
        if not self._auth(): return self._json(401,{"ok":False,"error":"AUTH_REQUIRED"})
        if self.path=="/v1/status":
            e,q=self.server.store.counts()
            return self._json(200,{"ok":True,"version":VERSION,"state_root":str(self.server.store.root),"event_count":e,"queued_commands":q,"model":model_status(),"single_durable_substrate":True,"normative_governor":"hakim_one_sovereign_kernel","external_providers":"optional_adapters"})
        if self.path.startswith("/v1/commands/"):
            cid=self.path.rsplit("/",1)[-1]
            cmd=self.server.store.get_command(cid)
            return self._json(200 if cmd else 404,{"ok":bool(cmd),"command":cmd} if cmd else {"ok":False,"error":"NOT_FOUND"})
        return self._json(404,{"ok":False,"error":"NOT_FOUND"})
    def do_POST(self):
        if self.client_address[0] not in ("127.0.0.1","::1"): return self._json(403,{"ok":False,"error":"LOOPBACK_ONLY"})
        if not self._auth(): return self._json(401,{"ok":False,"error":"AUTH_REQUIRED"})
        try: body=self._body()
        except Exception as e: return self._json(400,{"ok":False,"error":str(e)[:80]})
        if self.path=="/v1/events":
            evhash=self.server.store.event(body.get("kind","EVIDENCE"),body.get("source","client"),body.get("value",""))
            return self._json(200,{"ok":True,"evidence_sha256":evhash})
        if self.path=="/v1/commands":
            try:
                cid=self.server.store.put_command(body.get("intent",""),body.get("params",{}),body.get("idempotency_key",""))
...
def main():
    ...
    a=p.parse_args()
    if a.host not in ("127.0.0.1","::1","localhost"): raise SystemExit("LOOPBACK_ONLY")
    root=Path(a.state_dir).expanduser().resolve()
    s=Store(root)
    if a.print_token:
        print(s.token); return
    httpd=ThreadingHTTPServer(("127.0.0.1",a.port),Api); httpd.store=s
    def stop(*_): httpd.shutdown()
    signal.signal(signal.SIGTERM,stop); signal.signal(signal.SIGINT,stop)
    print(json.dumps({"ok":True,"version":VERSION,"bind":f"127.0.0.1:{a.port}","state":str(root)},ensure_ascii=False),flush=True)
    httpd.serve_forever()
