...
req('"ورقة_عمل_الجمع_ضمن_١٠"' in SPEC, "pdf_filename_missing")
req("easternDigits" in FACTORY and "toEastern" in FACTORY, "eastern_digits_missing")
req("problem.operation.symbol" in FACTORY, "operator_token_missing")
req("toEastern(problem.a)" in FACTORY and "toEastern(problem.b)" in FACTORY, "math_token_order_missing")
req("canvas.drawRect(box, line)" in FACTORY, "answer_box_missing")
req("HakimLocalArtifactFactory.canHandle(context, q)" in ROUTER, "local_artifact_detection_missing")
...
addition_block = SPEC[SPEC.index("fun additionWithinTen()"):SPEC.index("fun subtractionWithinTen()")]
pairs = [(int(a), int(b)) for a,b in re.findall(r"MathProblem\(\d+,\s*(\d+),\s*(\d+),\s*Operation.ADDITION\)", addition_block)]
req(len(pairs) == 8, "addition_problem_count")
req(all(a+b <= 10 for a,b in pairs), "sum_exceeds_ten")

needle = "HakimLocalArtifactFactory.canHandle(context, q)"
mutated = ROUTER.replace(needle, "false /* injected failure */", 1)
try:
