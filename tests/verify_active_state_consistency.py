...
req(state["cloud"]["last_verified_baseline_commit"] == "79114b2321e2534678dfceaf4417083e652e2d70", "cloud_baseline")
req(state["android"]["latest_source_parent"]["commit"] == "21166a98d062ccca9e3ed6851ca4b49f7d24e40e", "source_parent")
req(state["android"]["latest_source_parent"]["ci_run_number"] == 1146, "source_parent_ci")
req(state["android"]["candidate"]["version_code"] == candidate == 20307, "candidate_version")
...
req(state["local_execution"]["phone_channel"] == "ONLINE_DIRECT_HTTPS_VERIFIED", "phone_claim")
...
req(state["android"]["field_observed_current"]["evidence"] == "authenticated_direct_status_probe", "field_observed_evidence")
...
req(state["android"]["candidate"]["source_ci_state"] in {"PENDING_CURRENT_HEAD_CI", "SOURCE_CI_VERIFIED", "SUCCESS_CURRENT_HEAD_CI"}, "candidate_ci_state")
...
req(state["productization"]["candidate_version"] == candidate == 20307, "product_candidate")
...
req(state["productization"]["in_app_first"] is True, "in_app_first")
req(state["productization"]["pdf_visual_adapter"] is True, "pdf_visual_adapter")
req(state["productization"]["external_share_manual_only"] is True, "external_share_manual_only")

req(state["productization"]["sovereign_constitution_v4"] is True, "sovereign_constitution_v4")
req(state["productization"]["authority_boundary"] is True, "authority_boundary")
req(state["productization"]["external_content_data_not_commands"] is True, "external_content_data_not_commands")
req(state["productization"]["adaptive_value_budget"] is True, "adaptive_value_budget")
req(state["productization"]["fixed_cycle_ceiling"] is False, "fixed_cycle_ceiling")
req(state["productization"]["evidence_aware_closure"] is True, "evidence_aware_closure")
...
req(state["productization"]["control_channel_primary"] == "direct_https_secure_relay", "control_channel_primary")
req(state["productization"]["control_channel_urgent_recovery"] is True, "control_channel_urgent_recovery")
req(state["productization"]["control_channel_field_verified"] is True, "control_channel_field_verified")
req(state["productization"]["remote_desktop_commander_required"] is False, "rdc_must_not_be_required")
