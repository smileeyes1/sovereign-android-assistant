...
constraint_doctor = (ROOT / "app/src/main/java/ps/hakim/phoneagent/HakimConstraintDoctor.kt").read_text(encoding="utf-8")
...
# لا يكفي حذف الصلاحية من Manifest: لا يجوز لطبقات الاستعادة أن تعيد تحويلها
# إلى بوابة دائمة أو تدفع المستخدم لفتح مصادر غير معروفة كقدرة قائمة.
for source in [constraint_doctor, updater]:
    require("ACTION_MANAGE_UNKNOWN_APP_SOURCES" not in source,
            "P0: طبقة تشغيل تعيد مطالبة المصادر غير المعروفة رغم ملف التثبيت الآمن")
    require("canRequestPackageInstalls" not in source,
            "P0: طبقة تشغيل تعيد اعتماد تثبيت الحزم كصلاحية دائمة")
require("standing_unknown_source_permission_required" in constraint_doctor and "false" in constraint_doctor,
        "P0: طبيب القيود لا يعلن صراحة أن صلاحية المصادر غير المعروفة ليست قدرة دائمة مطلوبة")
