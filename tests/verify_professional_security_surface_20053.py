...
require(actual_permissions == expected_permissions,
        "تغيرت مجموعة الأذونات المعلنة في ٢٠٠٥٣ دون بوابة صريحة")

for activity in ("HakimProfessionalHubActivity", "HakimQuranActivity"):
    m = re.search(
        r'<activity\s+[^>]*android:name="\.' + activity + r'"[^>]*>',
        manifest,
        flags=re.S,
    )
    require(m is not None, "النشاط الاحترافي مفقود: " + activity)
    require('android:exported="false"' in m.group(0),
            "النشاط الاحترافي أصبح exported: " + activity)
...
require(actual_dependencies == expected_dependencies,
        "تغيرت اعتماديات التطبيق في ٢٠٠٥٣ دون تأهيل صريح")

require("versionCode 20053" in build, "بوابة الأمن مخصصة ل٢٠٠٥٣")
print("HAKIM_20053_PERMISSION_SURFACE=PASS")
print("HAKIM_20053_PRIVATE_ACTIVITIES=PASS")
print("HAKIM_20053_DEPENDENCY_SURFACE=PASS")
